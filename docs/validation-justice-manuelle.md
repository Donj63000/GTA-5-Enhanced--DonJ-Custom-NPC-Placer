...
| DOM-23 | Stabilité visuelle de l'admission | Reproduire DOM-05 jusqu'à obtenir la source `DurablePoliceDeath`. Filmer sans coupure le FadeOut initial et le placement en holding : aucun FadeIn intermédiaire n'est admis pendant que le WAL ou la sécurité policière attend encore. Le seul FadeIn survient après l'admission stable; rester ensuite 60 secondes dans l'enceinte sans provoquer une nouvelle mort ni en sortir. À 15 secondes, ouvrir F10, laisser le menu visible 10 secondes, puis le fermer. Le menu et le monde restent continuellement visibles après l'admission, sans nouveau fondu ni pulse noir de `0,4–0,5 s`; le temps de peine diminue dès le premier intervalle jouable. | NON EXÉCUTÉ | Vidéo continue ≥ 60 s + chronologie FadeOut/FadeIn + WAL/log source + capture F10/temps |
