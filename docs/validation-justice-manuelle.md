...
## Scénarios complémentaires de la revue Justice du 5 septembre 2026

| ID | Scénario | Procédure / résultat attendu | Résultat | Preuve |
|---|---|---|---|---|
| RVJ-01 | Litige après peine partiellement purgée | Résoudre le litige d'un dossier dont la peine est passée de 240 à 40 secondes et l'amende a été convertie. La peine reste à 40 secondes, les pénalités restent identiques, la dette reste nulle et aucun débit ne survient. Recharger et vérifier le même résultat. | NON EXÉCUTÉ | Dossier avant/après + XML + cash |
| RVJ-02 | Paiement OFF et profil inactif | Avec Justice OFF, payer partiellement puis intégralement une dette, recharger entre les essais et vérifier un seul débit, la dette exacte et OFF conservé. Consulter ensuite un autre héros portant un paiement ouvert : sa résolution de litige doit être refusée. | NON EXÉCUTÉ | Cash + profils + WAL/XML |
| RVJ-03 | Restitution différée et choix du joueur | Provoquer une restitution partielle, utiliser les munitions déjà rendues, changer la teinte et l'arme sélectionnée, puis jeter une arme rendue. Attendre plusieurs retries, utiliser la récupération F10 et recharger : seuls les éléments encore manquants sont repris, sans recréer l'arme jetée ni rétablir les anciens stocks et choix. | NON EXÉCUTÉ | Vidéo continue + marqueurs par arme |
| RVJ-04 | Writer retardé et backup | Retarder les écritures puis interrompre après un résultat de restitution présent uniquement dans le primaire. Sur une copie dédiée, reprendre depuis le backup et le même WAL : aucune restitution en double. Le snapshot reste présent tant que ses marqueurs ne sont pas prouvés dans les deux copies. | NON EXÉCUTÉ | Révisions XML/WAL + hashes + inventaire |
| RVJ-05 | Effet inventaire incertain | Injecter une exception après tentative de restitution. Si l'arme est absente et l'effet reste indécidable, le snapshot est conservé et aucun nouvel ajout aveugle n'est effectué, y compris après reload ou changement de héros. | NON EXÉCUTÉ | Appels natifs + diagnostic + WAL |
| RVJ-06 | Recognition inaccessible | Verrouiller successivement primaire, backup et une variante temporaire valide du store puis du journal critique. Aucun fichier n'est déplacé, vidé ou réécrit. Libérer le verrou : reprise automatique après le backoff, mêmes indices et commandes critiques. | NON EXÉCUTÉ | Hashes des six variantes + journal |
| RVJ-07 | Alliés et foule dense | Avec plus de douze civils et un policier placé en fin de snapshot, vérifier la progression de reconnaissance du policier. Un allié DonJ ne dénonce pas ; un civil recruté pendant son délai est retiré. Répéter avec gardes du corps et escortes. | NON EXÉCUTÉ | Exposition + groupes + vidéo + frametime |
| RVJ-08 | Victimes et véhicules au-delà du préfixe | Avec plus de huit cadavres et seize véhicules dont plusieurs hors des 80 m, provoquer un nouvel acte prouvé sur une cible en fin de liste. La rotation permet sa qualification, sans charge issue d'un dégât ancien ni requête monde supplémentaire. | NON EXÉCUTÉ | Incidents + causalité + compteurs |
| RVJ-09 | Évasion pendant riposte | S'évader depuis chacun des niveaux zéro à cinq étoiles, dont quatre/cinq pendant une riposte des gardes. Le niveau final vaut au moins trois et ne diminue jamais. | NON EXÉCUTÉ | Vidéo wanted + trace native |
...
| DOM-05 | Mort pendant poursuite | Filmer sans coupure une mort policière avec dossier actif et 3–5 étoiles. Le masque noir part pendant le ped mort, aucune frame jouable n'apparaît à l'hôpital, le même héros est maintenu dans l'enceinte pendant un WAL encore `Ambiguous`, puis une seule condamnation/capture l'envoie au bon site après confirmation primaire+backup, y compris si GTA conserve son flag WASTED/BUSTED. Répéter avec le débit d'amende encore `Prepared`, puis après un reload de cet état : la même transaction reprend une fois sans changement de héros. Au seul FadeIn final : wanted à zéro, aucune recherche plaque/tenue/planque ni mandat, gardes non hostiles, phase `Incarcerated` et peine déjà en progression sans changement de héros. Sur une copie avec amende intégralement payée et peine nulle, maintenir `IS_PLAYER_BEING_ARRESTED=true` après la sortie physique : `PendingDeathCapture` et `PendingLegalReleaseFinalization` restent durables, puis s'acquittent une seule fois après la première lecture fiable `false`, sans recréer de charge au tick suivant. | NON EXÉCUTÉ | Vidéo complète mort/maintien/prison + WAL/XML/log + HUD/temps/wanted |
...
| DOM-23 | Stabilité visuelle de l'admission | Reproduire DOM-05 avec Trevor à Bolingbroke, peine supérieure à cinq minutes, jusqu'à la source `DurablePoliceDeath`. Filmer sans coupure la mort, le FadeOut et le placement en holding : le primaire puis le backup acquièrent le front et le WAL devient `Confirmed` sans relancer le jeu ni changer de héros. Aucun FadeIn intermédiaire n'est admis; le seul FadeIn suit l'admission stable. Rester ensuite cinq minutes dans l'enceinte sans nouvelle mort ni sortie : aucune reprise de l'icône de chargement, aucun cycle noir supplémentaire, compteur actif. À 15 secondes, ouvrir F10 dix secondes, puis fermer. Répéter avec Michael et Franklin; vérifier aussi Mission Row jusqu'avant la libération légale. | NON EXÉCUTÉ | Vidéo continue mort + admission + ≥ 5 min à Bolingbroke, WAL/XML primaire et backup, logs et compteur |
| DOM-24 | Reprise du blocage de sauvegarde à l'admission | Sur une copie de test du profil Trevor, reprendre un état où le primaire contient le front de mort, le backup ne le contient pas et le WAL reste `Ambiguous`. Le chargement termine automatiquement les preuves redondantes et l'admission, sans reset du dossier, double condamnation, double débit ni seconde confiscation. Sur une copie distincte, retarder temporairement l'accès disque puis le rétablir : le joueur reste protégé pendant le refus et l'admission reprend après retour du writer, sans intervention sur les fichiers. | NON EXÉCUTÉ | Copie avant/après, WAL, logs, casier, argent et vidéo continue |
