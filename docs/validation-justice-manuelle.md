...
2. Je conserve une copie du dossier `DonJEnemySpawnerSaves`, notamment `_justice_state.xml`, son `.bak`, `_justice_state.wal` et toute quarantaine de reset. Cette copie legacy est obligatoire pour vérifier la remise à zéro unique sans risquer un profil réel.
...
| DOM-10 | Plafond de peine | Accumuler des infractions et une conversion d'amende au-delà de dix minutes : le dossier, la condamnation, le HUD et le XML restent tous plafonnés à `600` secondes. | NON EXÉCUTÉ | Détail F10 + HUD + XML |
...
| DOM-17 | Agression d'un garde | Frapper puis tuer un garde sans mourir : aucun fondu noir, déplacement, invincibilité, taser forcé, charge disciplinaire ou ajout de peine ne survient. La position et la peine restent inchangées; GTA gère seul les réactions locales. | NON EXÉCUTÉ | Vidéo continue + position/peine avant-après + trace natives/tâches |
...
| Combat en prison sans discipline, fade, invincibilité, tâche forcée ni téléportation | NON EXÉCUTÉ | — |
| Réactions GTA naturelles et maintenance après dix secondes de calme | NON EXÉCUTÉ | — |
| Mort seule renvoyant le détenu en cellule hors transferts normaux | NON EXÉCUTÉ | — |
...
| Barème divisé par trois, conversion 150 $/s et plafond strict de dix minutes | NON EXÉCUTÉ | — |
