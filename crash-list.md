...
## 2026-09-01 22:36:24 +02:00 - Validation prépublication interrompue par le jeu relancé
- Statut: Qualifié comme blocage environnemental; aucune régression de compilation, d'ABI ou de gameplay détectée.
- Contexte: Exécution finale de `tools\run-safety-checks.ps1 -UseStubApi` avant la publication GitHub, run `TestResults\safety-20260901-223202`.
- Symptôme: La suite termine à `740/742`; les deux seuls échecs sont les scénarios de déploiement transactionnel, qui refusent volontairement toute écriture dès qu'un processus GTA est actif.
- Sources vérifiées: `TestResults\safety-20260901-223202\logs\test-release.log`; `TestResults\safety-20260901-223202\safety-tests.trx`; rapport `bug-reports\20260901-223507-safety-failure`; liste des processus Windows.
- Extraits utiles: `GTA5_Enhanced` PID `34328`, démarré à `22:23:26`, et `PlayGTAV` PID `35592`, démarré à `22:23:00`; `GameReadyDeployment_ReplacesVerifiedFilesThenRemovesLegacyAliases` et `GameReadyDeployment_UsesNativeFallbackWhenHudRuntimeIsMissing` retournent tous deux `Déploiement refuse: ferme GTA et ses hosts de scripts avant toute modification de Scripts`.
- Analyse / hypothèse: Le jeu a été relancé après le précédent arrêt et avant cette passe. Le garde-fou de déploiement se comporte comme prévu; les 740 autres tests passent et aucun test Justice, Recognition, persistance, packaging ou documentation ne régresse.
- Action menée: Je n'ai ni interrompu le jeu de l'utilisateur ni contourné le verrou de sécurité. La publication reste conditionnée à la même suite exécutée dans GitHub Actions, où aucun processus GTA ne peut polluer ces deux scénarios.
- Vérification: Build Release stub zéro avertissement/zéro erreur; ABI NIB v2 valide sur 32 types et 189 membres; `740/742`, avec exactement les deux refus environnementaux attendus. La précédente passe locale propre avait réussi `742/742` dans `TestResults\safety-20260901-220319`, ainsi que `660/660` avec l'API réelle dans `TestResults\safety-20260901-221428`.
- Résolution: Incident local expliqué et contenu; le verrou de sécurité a empêché toute mutation de l'installation pendant que GTA tourne. La validation finale de publication sera celle du workflow GitHub Safety.

## 2026-09-01 23:48:51 +02:00 - Validation documentaire bloquée par la session GTA active
- Statut: Qualifié comme blocage environnemental; aucune régression de compilation, d'ABI ou de documentation détectée.
- Contexte: Exécution de `.\tools\run-safety-checks.ps1` après l'ajout de la capture Bolingbroke au README, run `TestResults\safety-20260901-234523`.
- Symptôme: La suite termine à `658/660`; seuls les deux scénarios de déploiement transactionnel refusent de s'exécuter pendant que GTA et son lanceur sont ouverts.
- Sources vérifiées: `TestResults\safety-20260901-234523\logs\test-release.log`; `TestResults\safety-20260901-234523\safety-tests.trx`; rapport `bug-reports\20260901-234825-safety-failure`; logs GTA, Scripts et événements Windows collectés par le rapport.
- Extraits utiles: `GTA5_Enhanced` PID `17756` et `PlayGTAV` PID `26936`; `GameReadyDeployment_ReplacesVerifiedFilesThenRemovesLegacyAliases` et `GameReadyDeployment_UsesNativeFallbackWhenHudRuntimeIsMissing` retournent `Déploiement refuse: ferme GTA et ses hosts de scripts avant toute modification de Scripts`.
- Analyse / hypothèse: Le garde-fou de déploiement détecte correctement la session de jeu active et interdit toute mutation, y compris vers le dossier GTA temporaire utilisé par les tests. Les 658 autres tests, la build Release et la validation ABI passent; l'ajout documentaire n'intervient dans aucun chemin runtime ou packaging.
- Action menée: Je n'ai ni fermé GTA ni contourné le verrou. L'image a été vérifiée octet pour octet, son PNG a été décodé et sa référence relative dans le README a été résolue localement.
- Vérification: Build Release sans avertissement ni erreur; ABI NIB v2 valide sur 32 types et 189 membres; `658/660`, avec exactement les deux refus environnementaux attendus. La validation complète est déléguée au workflow GitHub Safety déclenché par la publication, dans un environnement sans processus GTA.
- Résolution: Incident local expliqué et contenu; aucune installation GTA n'a été modifiée. La publication reste acceptable sous réserve du workflow GitHub Safety vert.

## 2026-09-03 20:49:21 +02:00 - Mort policière bloquée avant admission sûre en prison
- Statut: Correctif implémenté et validé hors jeu; incident reproduit et causes identifiées sur le binaire déployé `abc3b224101f8755798b998152b27215583f0587`. La clôture reste conditionnée au scénario manuel dans GTA.
- Contexte: Michael meurt pendant une poursuite à trois étoiles. Le maintien pré-jugement le replace à Bolingbroke, mais la capture et la peine ne deviennent actives qu'après une bascule Michael → Trevor → Michael. À la première arrivée visible, le wanted et la reconnaissance policière peuvent encore être actifs; les gardes attaquent alors le joueur.
- Symptôme: Le joueur apparaît en prison avec trois étoiles, est tué, puis revient en cellule. La peine et sa minuterie restent bloquées tant que le protagoniste n'est pas changé puis repris. Après la mort en détention, le writer peut en outre refuser le snapshot et suspendre durablement la progression.
- Sources vérifiées: rapport `bug-reports\20260903-202429-justice-death-prison-wanted-timer\crash-list-entry.md`; log copié `bug-reports\20260903-202429-justice-death-prison-wanted-timer\raw-logs\Grand-Theft-Auto-V-Enhanced__Scripts-2b01e185d936__DonJCustomNpcPlacer.log`; log copié `bug-reports\20260903-202429-justice-death-prison-wanted-timer\raw-logs\Grand-Theft-Auto-V-Enhanced__Scripts__DonJJusticeRecognitio-7dcc4ce4c679__JusticeRecognition.log`; sources `DonJEnemySpawner.Justice.cs`, `DonJEnemySpawner.Justice.Custody.cs`, `DonJEnemySpawner.Justice.Persistence.Runtime.cs` et codec/validateur Justice v2.
- Extraits utiles: à `20:15:59.712`, `Justice.MaintienAvantJugement` confirme `source=DurablePoliceDeath, slot=0, modèle=225514697`, mais `Justice.Capture` n'arrive qu'à `20:18:32.159`, juste après les changements Recognition vers Trevor à `18:18:11.249Z` puis Michael à `18:18:31.940Z`. L'identité est reliée à `20:18:32.245` et l'entrée Bolingbroke survient à `20:18:35.424`. Recognition avait créé une zone avec `WantedFloor=3` puis réappliqué trois étoiles à `18:15:52.599Z`. À `20:18:48.661`, le mod journalise une riposte gardiens à deux étoiles; à `20:19:03.701`, le writer refuse le snapshot avec `Invariants métier des profils Justice v2 invalides`, puis `Justice.MaintienDetention` annonce à `20:19:14.995` l'arrêt de la progression.
- Analyse / hypothèse: Le signal mission/BUSTED résiduel est traité comme une suspension générale avant la matérialisation du `DeathFront PoliceCapture` pourtant durable. Le bypass mission n'est armé qu'après un transfert stable, alors que le rebind nécessaire à ce transfert en dépend, créant une attente circulaire que le changement de héros casse fortuitement. Le fondu peut aussi être rendu avant la preuve wanted zéro, le reset Recognition durable et la quarantaine police, exposant le joueur et les gardes à l'ancienne poursuite. Enfin, une mort pendant la riposte peut sérialiser simultanément `GuardRetaliationActive=true` et les latches d'attente/rebind, combinaison rejetée par le validateur métier.
- Action menée: Collecte des logs avec `tools\collect-bug-logs.ps1`, corrélation de la chronologie Justice/Recognition, puis implémentation d'une admission transactionnelle fermée par défaut. Seuls le front `PoliceCapture`, l'identité canonique exacte et les preuves durables autorisent le bypass BUSTED résiduel. Le joueur reste masqué, figé et invincible jusqu'au reset Recognition durable, à l'inventaire confirmé, au wanted nul stable, à la quarantaine police et au retour effectif de l'écran. L'horloge démarre au même instant visible, sans changement de héros. Les mandats, zones, recherches de tenue/plaque/apparence/planque et fronts de riposte hérités sont annulés; les échecs du writer réarment la transaction sans exposer le joueur.
- Vérification: Tests ciblés des régressions historiques et des nouvelles frontières réussis. `.\tools\run-safety-checks.ps1 -UseStubApi` réussit `788/788` dans `TestResults\safety-20260904-100137`, avec build Release, validation ABI NIB v2 sur 32 types et 189 membres et package vérifié. `dotnet build GTA5modDEV.sln -c Release` réussit sans avertissement ni erreur avec l'API locale; `dotnet test GTA5modDEV.sln -c Release` réussit `665/665`. `git diff --check` ne signale aucune erreur de whitespace. Le scénario manuel reste à effectuer : mort policière avec étoiles, aucun changement de héros, premier écran rendu à zéro étoile, aucune recherche Recognition, aucune hostilité de garde héritée et décrément de la peine après une seconde.
- Résolution: Correctif validé automatiquement et prêt à livrer. L'incident reste ouvert uniquement jusqu'à la validation manuelle GTA du binaire installé; toute étoile, recherche Recognition, attaque héritée ou minuterie immobile après le premier écran visible constitue une régression.

## 2026-09-04 10:09:49 +02:00 - Échecs intermédiaires pendant le durcissement de l'admission Justice
- Statut: Résolus avant la validation finale; aucun binaire intermédiaire n'a été livré.
- Contexte: Développement et exécutions ciblées du correctif de mort policière, d'admission carcérale et de libération sans cellule.
- Symptôme: Les premières passes ont successivement signalé une assertion source obsolète (`777/778`), une boucle d'acquittement après `85` tentatives, deux erreurs temporaires `CS0103` pendant l'ajout coordonné des helpers, puis des écarts ciblés sur l'horloge d'admission, l'identité personnalisée divergente, le comptage de paramètres par réflexion et la reprise sans cellule. Une tentative manuelle avait aussi combiné à tort `UseStubApi=true` avec le `GtaRoot` réel, produisant des erreurs de compilation étrangères au code livré.
- Sources vérifiées: sorties directes MSBuild/VSTest; `TestResults\safety-20260904-095159`; `JusticePoliceDeathHospitalRespawnTests.cs`; `JusticeCustodyAdoptedRespawnIdentityTests.cs`; `StubRuntimeBehaviorTests.cs`; sources Justice concernées. Quelques recherches `rg` avec jokers Windows ont également retourné l'erreur système `123`, sans impact produit.
- Extraits utiles: La passe `safety-20260904-095159` isolait trois contrats encore faux : attente writer absente dans le scénario FadeIn refusé, modèle personnalisé divergent accepté comme source capturée et signature de helper non réalignée. Le test strict d'horloge observait auparavant `750` au lieu de `0` avant le FadeIn final.
- Analyse / hypothèse: Ces échecs ont exposé des frontières de transaction incomplètes et quelques fixtures devenues obsolètes pendant l'implémentation. Les erreurs `CS0103`, la mauvaise combinaison de propriétés MSBuild et l'erreur `rg` relevaient uniquement des commandes intermédiaires.
