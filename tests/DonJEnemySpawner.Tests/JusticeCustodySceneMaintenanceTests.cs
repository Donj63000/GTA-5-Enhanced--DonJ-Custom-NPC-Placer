...
{
    [TestMethod]
    public void CustodyScene_ReplacesTheVacantStableSlotInsteadOfDuplicatingTheLastPost()
    {
        Assert.AreEqual(
            1,
            DonJEnemySpawner.SelectJusticeCustodyReplacementSlot(4, 4, 1),
            "La perte du poste 1 doit recréer le poste 1, pas un second poste 3.");
        Assert.AreEqual(
            3,
        ...
        Assert.IsFalse(
            compaction.Contains("peds.RemoveAt(index)"),
            "La compaction ne doit plus décaler les indices liés aux postes.");
        ...
        StringAssert.Contains(scene, "_justiceCustodyGuards[guardIndex] = guard");
        StringAssert.Contains(scene, "_justiceCustodyInmates[inmateIndex] = inmate");
    }
    ...
    [TestMethod]
    public void CustodyScene_ReturnOrdersAreCadencedAndRestartAfterDiscipline()
    {
        ...
        string disciplineEnd = ExtractMethodBody(
            source,
            "EndJusticeCustodyDiscipline");
