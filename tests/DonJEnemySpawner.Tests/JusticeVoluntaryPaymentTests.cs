...
public sealed class JusticeVoluntaryPaymentTests
{
    ...
    public void VoluntaryPayment_PaysAvailableCashOnceAndKeepsTheRemainingDebt()
    {
        WithTemporarySaveDirectory(directory =>
        {
            ...
            Assert.IsTrue(
                ...
                "Le profil de base du test doit être persistable avant le WAL.");
    ...
    public void VoluntaryPayment_UnreadableCashAfterUnknownWriteTimesOutWithoutReplay()
    {
        WithTemporarySaveDirectory(directory =>
        {
            ...
            Assert.AreEqual(0L, GetCase(script).FineDue);
            Assert.AreEqual(600L, GetCase(script).VoluntaryFinePaid);
            Assert.IsNull(GetField<object>(script, "_justiceVoluntaryFinePaymentIntent"));
            ...
            Assert.AreEqual(600L, GetCase(reloaded).VoluntaryFinePaid);
    ...
    public void VoluntaryPayment_PartialAndFullBalancesRemainValidThroughCaptureAndReload()
    {
        WithTemporarySaveDirectory(directory =>
        {
            ...
            Assert.IsTrue(
                (bool)Invoke(writer, "JusticeFlushStateNow"),
                "Le jugement doit accepter le reliquat après paiement partiel.");
            ...
            Assert.IsTrue(
                (bool)Invoke(writer, "JusticeFlushStateNow"),
                "Le jugement doit accepter une dette entièrement payée avant capture.");
    ...
    public void VoluntaryPayment_CancelledPreparedIntentCannotBeResurrectedFromBackup()
    {
        WithTemporarySaveDirectory(directory =>
        {
            ...
            // Je laisse le WAL préparé dans le primaire et le backup, mais je
            // bloque sa première reprise avant toute lecture ou écriture cash.
            ...
            Assert.IsTrue((bool)Invoke(writer, "ResumeJusticeVoluntaryFinePayment"));
            ...
            Assert.IsFalse(
                ...
                "Le backup ne doit conserver aucune intention encore débitable après l'annulation.");
            ...
            Assert.IsTrue((bool)Invoke(reloaded, "TryLoadJusticeState", false));
            Assert.IsNull(GetField<object>(reloaded, "_justiceVoluntaryFinePaymentIntent"));
            Assert.AreEqual(600L, GetCase(reloaded).FineDue);
            Assert.IsTrue((bool)Invoke(reloaded, "ResumeJusticeVoluntaryFinePayment"));
    ...
    [TestMethod]
    public void VoluntaryPayment_SecondPrecommitFailureReloadsPersistedIntentIdempotently()
    {
        WithTemporarySaveDirectory(directory =>
        {
            int cash = 1000;
            ...
            Assert.IsNotNull(GetField<object>(writer, "_justiceVoluntaryFinePaymentIntent"));
            string statePath = Path.Combine(directory, "_justice_state.xml");
            Assert.IsTrue(File.Exists(statePath), "La première écriture du précommit doit exister.");
            StringAssert.Contains(File.ReadAllText(statePath), "VoluntaryFinePaymentIntent");
            StringAssert.Contains(GetField<string>(writer, "_statusText"), "Paiement en attente");

            object reloaded = CreatePaymentScript(1L);
            SetField(reloaded, "_justiceCashReadOverride", new Func<int, int?>(slot => cash));
            SetField(
                reloaded,
                "_justiceCashWriteOverride",
                new Func<int, int, bool?>((slot, value) =>
            ...
            Assert.IsTrue((bool)Invoke(reloaded, "TryReadJusticeStateFile", statePath));
            Assert.IsNotNull(GetField<object>(reloaded, "_justiceVoluntaryFinePaymentIntent"));
    ...
    public void JusticePersistence_AFailureIsRateLimitedAndRetriesAfterOneSecond()
    {
        WithTemporarySaveDirectory(directory =>
        {
            ...
            Assert.IsFalse((bool)Invoke(script, "JusticeFlushStateNow"));
            Assert.AreEqual(
                1100L,
